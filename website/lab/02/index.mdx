...
|-|-|
| `wait_for_high` | Suspends the execution until the pin state becomes `PinState::High` |
| `wait_for_low` | Suspends the execution until the pin state becomes `PinState::Low` |
| `wait_for_rising_edge` | Suspends the execution until the pin state switches from `PinState::Low` to `PinState::High` |
| `wait_for_falling_edge` | Suspends the execution until the pin state switches `PinState::High` to `PinState::Low` |
...
| Position | Value |
|-|-|
| Pressed | `PinState::Low` |
| Released | `PinState::High` |
...
Make sure you connect the switch SW5 to a Raspberry Pi Pico 2 pin. (**2p**)
...
7. Build a traffic light using the `GREEN`, `YELLOW` and `RED` LEDs. The flow of the colors
is:
...
8. Extend the traffic light to include a flashing blue LED for pedestrians.
 ...
 and waits one second. After that, oit switches on the red light and flashed
...
One of the PAC crates for the RP2040 MCU is [rp2350-pac](https://docs.rs/rp2040-pac/latest/rp2350_pac/).
...
The RP2350 (ARM Cortex-M33) MCU uses *Memory Mapped Input Output* (MMIO). This means that the peripheral's registers
...
1. Write a firmware using PAC that blinks the green LED every seconds.
2. Write a firmware using bare metal that blinks the green LED every seconds.