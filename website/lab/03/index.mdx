...
RP2350 provides two tiner peripherals, while RP2040 provides only one.
:::
...
A *counter* is used by the RP2350 and RP2040 to generate the PWM signal. The PWM counter is controlled by these registers (`X` can be from 0-7, depending on the channel):
...
The servo motor that the board provides is signaled with labels `GND`, `PWR` and `SIG` in the connectors section.

![ServoMotor](images/board_servo.png)

To wire the servo motor to the  Raspberry Pi Pico 2, a jumper wire is needed to connect the `SERVOS` connector to PWM pins on the board.
...
```rust
    ...
    // Servos typically expect a 20 ms period (50 Hz), so we calculate the TOP value accordingly.
    // The formula for TOP is: TOP = (Clock Frequency / (PWM Frequency * Divider)) - 1
    // For 150 MHz clock, 50 Hz frequency, and a divider of 64:
    // TOP = (150,000,000 / (50 * 64)) - 1 = 46,874 (0xB71A in hex)
    servo_config.top = 0xB71A; // TOP value for 50 Hz frequency

    // Set the clock divider to 64
    // The RP2350 runs at 150 MHz, which is too fast for direct PWM control.
    // Dividing the clock by 64 brings it down to a manageable frequency.
    ...
    // The formula is: compare_value = (pulse_width_us * TOP) / period_us
    // This scales the pulse width to the PWM counter range.
...
```rust
    ...
    let mut servo = Pwm::new_output_a(peripherals.PWM_SLICE1, peripherals.PIN_2, servo_config.clone());
