...
RP2350 MCU. The lab presents three examples:
 ...
 - **embassy-rs** - using the Rust standard [`embedded-hal`](https://docs.rs/embedded-hal/latest/embedded_hal/) implemented by the [Embassy](https://embassy.dev/) framework.
...
The name *Embassy* is derived form **Emb**edded **Asy**nchronous Rust.
...
This lab makes you of the hardware available on the lab board. The board provides:
...
The board provides three single colored LEDs, red, green and blue. Each one of them
uses one pin for control. Each LED connector has one single hole on the board,
marked with `RED`, `GREEN` and `BLUE` respectively. These are located in the **Connectors**
section of the board.

The fours switches that the lab board provides are signaled with labels
...
2. Pins that connect to switches have to be set up as `Pull::Up`.
:::

### Example

To wire the green LED to pin 0 of the Raspberry Pi Pico 2, a jumper cable is required
between holes `GREEN` and `GP0`.
...
2. Write a program using Embassy that blinks the LED connected to GPIO pin 0 (GP0) 
every 300ms. (**2p**)
...
3. Write a program using Embassy that will write the
...
4. Write a Rust program using Embassy that toggles the LED every time button SW5 is pressed. (**1p**)

5. Instead of constantly sampling for the button value, use the [`wait_for_`](https://docs.embassy.dev/embassy-rp/git/rp2040/gpio/struct.Input.html#method.wait_for_low) functions to 
