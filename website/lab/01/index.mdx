...
located in the root of the firmware'sw crate.

:::warning
The `build.rs` file is located in the root of the firmware's crate, not in the `src` folder.
:::

The `build.rs` file compiled before the firmware and is executed by the Rust compiler during
the compilation. The `build.rs` file write to the *screen* compiler options that the compiler
