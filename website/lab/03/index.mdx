...
$$
\text{TOP} = \left( \frac{\text{Clock Frequency}}{\text{PWM Frequency} \times \text{Divider}} \right) - 1
$$
...
$$
\text{TOP} = \left( \frac{150,000,000}{50 \times 64} \right) - 1 = 46,874
$$

which is **0xB71A** in hexadecimal.


The **clock divider** is used to slow down the high-frequency system clock so that it can generate a usable PWM signal. The RP2350's system clock runs at 150 MHz, which is too fast for direct PWM control. By setting the divider to 64, we effectively slow down the clock.
...
$$
\text{compare\_value} = \left( \frac{\text{pulse\_width\_us} \times \text{TOP}}{\text{PERIOD\_US}} \right)
$$

where:
- `pulse_width_us` is the desired pulse width in microseconds.
- `TOP` is the previously calculated counter value (46,874).
- `PERIOD_US` is the total period in microseconds (20,000 μs for 50 Hz).
